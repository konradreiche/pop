...
func (q *Query) Join(table string, on string, args ...interface{}) *Query {
	if q.RawSQL.Fragment != "" {
		Logger.WithField("raw", q.RawSQL.Fragment).WithField("table", table).WithField("on", on).WithField("args", args).Warn("Query is setup to use raw SQL, not adding JOIN clause")
		return q
	}
